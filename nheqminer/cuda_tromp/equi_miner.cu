...
#ifdef XINTREE
			const u32 xhash = (ph[1] & 0xf) << 4 | ph[2] >> 4;
#endif
	...
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		...
		for (u32 s1 = 0; s1 < bsize; s1++) {
			...
			for (; cd.nextcollision();) {
				...
				xorbucketid = ((u32)(bytes0[htl.prevbo] ^ bytes1[htl.prevbo]) << 4)
					| (bytes0[htl.prevbo + 1] ^ bytes1[htl.prevbo + 1]) >> 4;
				xhash = (bytes0[htl.prevbo + 1] ^ bytes1[htl.prevbo + 1]) << 4
					| (bytes0[htl.prevbo + 2] ^ bytes1[htl.prevbo + 2]) >> 4;
				xhash &= 0xff;
#else
#error not implemented
	...
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		...
		for (u32 s1 = 0; s1 < bsize; s1++) {
			...
			for (; cd.nextcollision();) {
				...
				xorbucketid = ((u32)(bytes0[htl.prevbo] ^ bytes1[htl.prevbo]) << 4)
					| (bytes0[htl.prevbo + 1] ^ bytes1[htl.prevbo + 1]) >> 4;
				xhash = bytes0[htl.prevbo + 1] ^ bytes1[htl.prevbo + 1];
				xhash &= 0xff;
#else
#error not implemented
#endif
	...
	: threadsperblock(tpb), totalblocks(blocks), device_id(id)
{
	...
	checkCudaErrors(cudaDeviceSetCacheConfig(cudaFuncCachePreferNone));
