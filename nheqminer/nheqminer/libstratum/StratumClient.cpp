...
StratumClient<Miner, Job, Solution>::StratumClient(
		std::shared_ptr<boost::asio::io_context> io_s, Miner * m,
...
void StratumClient<Miner, Job, Solution>::workLoop()
{
    ...
    while (m_running) {
        try {
            if (!m_connected) {
                //m_io_service.run();
                //boost::thread t(boost::bind(&boost::asio::io_context::run, &m_io_service));
                connect();
...
void StratumClient<Miner, Job, Solution>::connect()
{
    ...
    auto endpoints = r.resolve(p_active->host, p_active->port);
