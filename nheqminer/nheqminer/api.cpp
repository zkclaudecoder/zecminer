...
API::API(std::shared_ptr<boost::asio::io_context> io_service)
...
{
	boost::asio::ip::tcp::endpoint endp(boost::asio::ip::make_address("127.0.0.1"), local_port);
...
bool API::poll()
{
	try
	{
		if (m_io_service->poll() > 0) return true;
		else return false;
	}
